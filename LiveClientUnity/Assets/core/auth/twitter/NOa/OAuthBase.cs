...
namespace Narangec.Twitter
{
  public class OAuthBase
  {
    ...
    public string GenerateSignatureBase(Uri url, string consumerKey, string token, string tokenSecret, string httpMethod, string timeStamp,
      string nonce, string OAuthVerifier, string signatureType, out string normalizedUrl, out string normalizedRequestParameters)
    {
      if (token == null) {
        token = string.Empty;
      }
      ...
      string timeStamp, string nonce, SignatureTypes signatureType, out string normalizedUrl, out string normalizedRequestParameters)
    {
      ...
      switch (signatureType) {
        case SignatureTypes.PLAINTEXT:
          return UnityWebRequest.EscapeURL(string.Format("{0}&{1}", consumerSecret, tokenSecret), Encoding.UTF8);
        case SignatureTypes.HMACSHA1:
          string signatureBase = GenerateSignatureBase(url, consumerKey, token, tokenSecret, httpMethod, timeStamp, nonce, string.Empty,
            HMACSHA1SignatureType, out normalizedUrl, out normalizedRequestParameters);
    ...
    /// <param name="httpMethod">The http method used. Must be a valid HTTP method verb (POST,GET,PUT, etc)</param>
    /// <returns>A base64 string of the hash value</returns>
    public string GenerateSignatureWithOAuthVerifier(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, /* JDevlin*/ string OAuthVerifier,
      out string normalizedUrl, out string normalizedRequestParameters)
    {
      return GenerateSignatureWithOAuthVerifier(url, consumerKey, consumerSecret, token, tokenSecret, httpMethod, timeStamp, nonce, OAuthVerifier, 
    ...
    public string GenerateSignatureWithOAuthVerifier(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp,
      string nonce, string OAuthVerifier /*JDevlin*/, SignatureTypes signatureType, out string normalizedUrl, out string normalizedRequestParameters)
    {
      normalizedUrl = null;
      normalizedRequestParameters = null;
      ...
      switch (signatureType) {
        case SignatureTypes.PLAINTEXT:
          return UnityWebRequest.EscapeURL(string.Format("{0}&{1}", consumerSecret, tokenSecret));
        case SignatureTypes.HMACSHA1:
          string signatureBase = GenerateSignatureBase(url, consumerKey, token, tokenSecret, httpMethod, timeStamp, nonce, OAuthVerifier, 
            HMACSHA1SignatureType, out normalizedUrl, out normalizedRequestParameters);
